...
class MusicBrainzService
{
    ...
    {
        $cache = Cache::get('mb_track_'.$recordingId);
        ...
        foreach ($medias as $media) {
            ...
            foreach ($tracks as $track) {
                if ($track['recording']['id'] == $recordingId) {
                    Cache::put('mb_track_'.$track['id'], $track);
    ...
    public function getRecording(string $id): array
    {
        ...
        $response = $this->getHttp()->get($this->apiUrl.'/recording/'.$id.'?inc=artist-credits+isrcs+annotation+tags+genres+releases+release-groups&fmt=json');
